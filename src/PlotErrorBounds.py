...
def compute_bound_Pii(network, P, i, Ustar, Vstar, Sigma, Phi_stacked):
    ### i is the index-1.
    ...
    sv_Phi_hat, _ = scipy.linalg.eigh(Phi_hat[:network.plunging_dimension].T @ Phi_hat[:network.plunging_dimension])
    eig_Phi_hat_star, _ = scipy.linalg.eigh(Phi_hat[:network.rank_S].T @ Phi_hat[:network.rank_S])
...
def compute_theoretical_bounds(network, eta):
    ...
    r = network.plunging_dimension
    S_stacked = np.concatenate([client.S for client in network.clients])
    if network.power % 2 == 0:
        Phi_stacked = network.clients[0].Phi_V # In this case, all Phi are identical.
    else:
        Phi_stacked = np.concatenate([client.Phi_V for client in network.clients])
    ...
    for i in range(network.plunging_dimension+1, min(network.nb_clients * network.nb_samples, network.dim)):
        upper_Pii = compute_bound_Pii(network, P, i, Ustar, Vstar, Sigma, Phi_stacked)
...
def plot_errors_vs_condition_number(nb_clients: int, nb_samples: int, dim: int, rank_S: int, latent_dim: int,
    ...
    init_legend = []
    init_legend.append(Line2D([0], [0], linestyle="-", color='tab:blue', lw=2, label=r'$\|\Sigma - \Sigma P \|^2_F / 2$'))
...
if __name__ == '__main__':
    ...
    # Without noise.
    plot_errors_vs_condition_number(10, 50, 200, 5, 5, 0, 0,
                                    0)
